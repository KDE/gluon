...
void GraphPropertiesWidget::setEdgeDefaultColor( QColor c )
{
    emit edgeDefaultColorSetted( c.name() );
}
...
{
    emit edgeColorsChanged( _graphEdgeColor->color().name() );
}
void GraphPropertiesWidget::on__graphNodeColorApplyNow_clicked()
{
    emit nodeColorsChanged( _graphNodeColor->color().name() );
}
