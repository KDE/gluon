...
AtomicFile::AtomicFile(const QString &filename)
    : d(new Private)
{
    AtomicFile::setFileName(filename);
}
...
bool AtomicFile::open(OpenMode flags)
{
    ...
    if (!GluonPlayer::checkAccess(d->realFileName, W_OK)) {
...
void AtomicFile::setFileName(const QString &filename)
{
    ...
    QDir realFileNameDir( d->realFileName );
    QString tmpFileName = realFileNameDir.canonicalPath( );
    if( !tmpFileName.isNull() )
        d->realFileName = tmpFileName;

    return;
}

QFile::FileError AtomicFile::error() const
{
...
QString AtomicFile::errorString() const
{
...
QString AtomicFile::fileName() const
{
...
bool AtomicFile::finalize()
{
    ...
    if ( !d->isFinalized ) {
        ...
        //race condition.
        else if (0 == GluonPlayer::rename(d->tempFileName,d->realFileName)) {
            d->error = QFile::NoError;
            d->errorString.clear();
            success = true;
        } else {
...
// bool AtomicFile::backupFile( const QString& qFilename, const QString& backupDir )
// {
...
bool AtomicFile::simpleBackupFile( const QString& qFilename,
...
//    qDebug() << "AtomicFile copying " << qFilename << " to " << backupFileName;
    QFile::remove(backupFileName);
    return QFile::copy(qFilename, backupFileName);
}

// bool AtomicFile::rcsBackupFile( const QString& qFilename,
                               // const QString& backupDir,
...
bool AtomicFile::numberedBackupFile( const QString& qFilename,
                                   const QString& backupDir,
                                   const QString& backupExtension,
...
//        kDebug(180) << "AtomicFile renaming " << from << " to " << to;
        QFile::rename( from, to );
        to = from;
    }

    // Finally create most recent backup by copying the file to backup number 1.
//    qDebug() << "AtomicFile copying " << qFilename << " to " << sTemplate.arg(1);
