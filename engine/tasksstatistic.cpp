...
void TasksStatistic::accomplished( qlonglong index )
{
    if( d->array.contains( index ) )
        return;

    for( QList<qlonglong>::iterator iter = d->array.begin(); iter != d->array.end(); ++iter )
    {
        if( *iter > index )
        {
            d->array.insert( iter, index );
            return;
        }
    }
...
void TasksStatistic::notAccomplished( qlonglong index )
{
