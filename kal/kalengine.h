...
static KALEngine * getInstance(Phonon::Category category);
  ...
  int sourceCount(){return m_sourceList.size();}

void addSource(KALSource * source){m_sourceList.append(source);}
void removeSource(KALSource * source){m_sourceList.removeOne(source);}

void addBuffer(KALBuffer * buffer){m_bufferList.append(buffer);}
void removeBuffer(KALBuffer * buffer){m_bufferList.removeOne(buffer);}
